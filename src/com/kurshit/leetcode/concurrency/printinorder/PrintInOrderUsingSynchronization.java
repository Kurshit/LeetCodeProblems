...
/*
 * SOLUTION 4 : Using Synchronization
 */

class FooSynchronization {

	private String mutex = new String("");
	private int counter = 1;

	public FooSynchronization() {
...
public class PrintInOrderUsingSynchronization {

	public static void main(String[] args) {

		FooSynchronization foo = new FooSynchronization();
	...
	private static class CallsFirst implements Runnable {
		private FooSynchronization foo;
		private PrintsFirst first;

		public CallsFirst(FooSynchronization foo, PrintsFirst first) {
			this.foo = foo;
			this.first = first;
		}
	...
	private static class CallsSecond implements Runnable {
		...
		public CallsSecond(FooSynchronization foo, PrintsSecond seconds) {
			this.foo = foo;
	...
	private static class CallsThird implements Runnable {
		...
		public CallsThird(FooSynchronization foo, PrintsThird thirds) {
			this.foo = foo;
			this.thirds = thirds;
		}
